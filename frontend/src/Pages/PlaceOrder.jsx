...
const {cartItem , setCartItem , getCartAmount , delhivery_fee , product } = useContext(shopDataContext)
    ...
    try {
      ...
      for(const items in cartItem){
       ...
       let orderData = {
        address:formData,
        items:orderItems,
        amount:getCartAmount() + delhivery_fee
      }
...
const onChangeHandler = (e)=>{
 const name = e.target.name;
  ...
  return (
    ...
    bg-gradient-to-l from-[#141414] to-[#0c2025] flex items-center justify-center flex-col md:flex-row gap-[50px]  relative'>
       ...
       <div className='py-[10px]'>
        <Title text1={'DELIVERY'} text2={'INFORMATION'}/>
         ...
         <input type="text" placeholder='First name' className='w-[48%] h-[50px] rounded-md
          bg-slate-700 placeholder:text-[white] text-[18px] px-[20px] shadow-sm
           shadow-[#343434]'
           required 
         ...
         />
          ...
          <input type="text" placeholder='Last name' className='w-[48%] h-[50px]
           rounded-md shadow-sm shadow-[#343434] bg-slate-700 placeholder:text-[white] 
          ...
          <input type="email"
           ...
           className='w-[100%] h-[50px] rounded-md shadow-sm shadow-[#343434]
          ...
          <input type="text"
           placeholder='Street'
            className='w-[100%] h-[50px] rounded-md
             bg-slate-700 shadow-sm shadow-[#343434]
            ...
            <div className='lg:w-[70%] w-[90%] lg:h-[70%] h-[100%]  flex items-center justify-center gap-[10px] flex-col'>
              ...
              <div className='w-[100%] h-[30vh] lg:h-[100px] flex items-start mt-[20px] lg:mt-[0px] justify-center gap-[50px]'>
                 <button onClick={()=>setMethod('razorpay')} className={`w-[150px] h-[50px] rounded-sm  ${method === 'razorpay' ? 'border-[5px] border-blue-900 rounded-sm' : ''}`}> <img src={Razorpay} className='w-[100%] h-[100%] object-fill rounded-sm ' alt="" /></button>
                  <button onClick={()=>setMethod('cod')} className={`w-[200px] h-[50px] bg-gradient-to-t from-[#95b3f8] to-[white] text-[14px] px-[20px]
                   rounded-sm text-[#332f6f] font-bold ${method === 'cod' ? 'border-[5px] border-blue-900 rounded-sm' : ''}`}>CASH ON DELIVERY </button>
              </div>
